"""
Pose-only MLP inference with auto-reset per episode
conda activate maniskill2 && python step8_poseonly_infer.py
"""
import sapien, numpy as np, torch, torch.nn as nn
from scipy.spatial.transform import Rotation

URDF   = '/home/rishabh/Downloads/myarm_m750_fixed.urdf'
OUT    = '/home/rishabh/Downloads/umi-pipeline-training/outputs/poseonly_model'
DEVICE = 'cuda:0'
PRED_STEPS=8; N_DOFS=7
EPISODE_LEN = 360   # same as training episodes

mean = np.load(f'{OUT}/norm_mean.npy')
std  = np.load(f'{OUT}/norm_std.npy')

class PoseNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(N_DOFS,512),nn.ReLU(),
...
def reset_episode():
    global grasped, step, queue, placed
    # Random marker and box positions like training data
...
grasped=False; step=0; queue=[]; placed=False
mx,my,bx,by = reset_episode()

print("Running inference — model controls robot!")
while not viewer.closed:
    # Auto reset after episode ends
    if step >= EPISODE_LEN:
        episode += 1
        mx,my,bx,by = reset_episode()
        continue
    ...
    obs=np.concatenate([ep_p,rot_aa,grip_w])
    ...
    if not queue:
        ...
        acts=(pred.reshape(PRED_STEPS,N_DOFS)*std)+mean
        queue=list(acts)
        if step%40==0:
    ...
    # Grasp/place logic
    mpos=np.array(sim_marker.get_pose().p)
    ...
    if grasped and grip>0.025 and not placed:
        ...
        if np.linalg.norm(mp[:2]-bp[:2])<0.12:
            ...
            print(f"  📍 Released at ({mp[0]:.3f},{mp[1]:.3f}) step={step}")
